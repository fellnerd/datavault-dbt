...
{#-
  Staging: Sharepoint Forecast (Planungsdaten)
  Source: ext_ewb_sp_forecast_json (JSON via OPENJSON)
  Business Key: Datum + Kostenstelle + Konto + Szenario
  DV Role: Mart-level enrichment (Forecast vs Actual)
-#}
...
FROM {{ source('staging', 'ext_ewb_sp_forecast_json') }} AS r
