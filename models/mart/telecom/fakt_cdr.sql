...
{{ config(
    ...
    unique_key='hk_link_cdr_event_tl',
    as_columnstore=false,
    tags=['fact'],
    post_hook=[
        "{{ create_hash_index('verbindungs_datum_key') }}",
        "{{ create_hash_index('vertrag_key') }}",
        "{{ create_hash_index('kunde_key') }}"
    ]
) }}
...
SELECT
    -- Foreign Keys (Surrogate Keys der Dimensionen, persistent gespeichert)
