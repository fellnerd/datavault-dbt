/*
 * Faktentabelle: fakt_cdr_v
 * Schema: mart_telecom
 *
 * Atomarer CDR-Grain: 1 Zeile = 1 Call Detail Record (Anruf / Daten-Session / SMS).
 * Zeigt alle im Vault verfügbaren CDR-Events (kein Zeitfilter).
 * Nach Aktivierung der 30-Tage-Retention-Purge (§13.8 Implementierungsplan)
 * enthält der Vault nur noch die letzten 30 Tage.
 *
 * Metriken:
 *   duration_sec   — Verbindungsdauer in Sekunden (Anrufe / Datensessions)
 *   bytes_out_mb   — Übertragenes Datenvolumen in MB (nur DATA-Events)
 *   price          — Verbrauchskosten (aus Compax)
 *   ws_price       — Wholesale-Preis
 *
 * Für aggregierte BI-Abfragen: fakt_datenvolumen_v / fakt_anrufe_v verwenden.
 *
 * Quellen:
