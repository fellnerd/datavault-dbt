/*
 * Dimension: dim_konto (materialisierter Performance-Cache)
 * Schema: mart_finance
 *
 ...
 *   dieselbe Regel speist auch die Power-BI-Calculation-Group "Summary Lines").
 ...
 * Grund: dim_konto_v war als View bei jedem DirectQuery-Aufruf neu zu berechnen
 * (3x UNION ALL + TRY_CAST/HASHBYTES), gemessen ~2.3-2.6s Zusatzkosten pro Aufruf
 * bei nur 449 Zeilen (Performance-Analyse 2026-07-24). Konsumenten nutzen weiterhin
 * dim_konto_v (Wrapper-View, publiziertes Objekt) — analog zum fakt_buchungen-Muster.
 *
 * Zeilentyp (Zebra BI "Category Class"-Bucket, Symbole: "=" Result, "-" Invert, "/" Skip):
 *   NULL fuer alle echten Konten + Gruppen-Plugs (normale Detailzeilen).
 *   "=" fuer die 6 Summary-Plugs (Zwischensummen-Zeilen) — Zebra BI berechnet den Wert
 *   dann selbst aus den Zeilen darueber, unabhaengig vom (bei Plug-Zeilen leeren) Fakt-Join.
 *   "/" fuer "x Hilfskonten" (kein Bestandteil irgendeiner P&L-Summenzeile).
 *
 * Version: 2026-07-24 V1.1 Spalte zeilentyp ergaenzt (Zebra BI Category Class)
 */
...
enriched AS (
    SELECT
        ...
        -- Zebra BI "Category Class"-Bucket (Result/Invert/Skip, siehe Header-Kommentar):
        -- "x Hilfskonten" wird geskippt (fliesst in keine P&L-Summenzeile ein, wie ab_stufe=NULL
        -- in konto_pl_zuordnung_v), alle anderen echten Konten sind normale Detailzeilen (NULL).
        CASE WHEN z.konto_l2_prefix = 'x' THEN '/' ELSE NULL END                    AS zeilentyp,
        b.dss_load_date,
        b.dss_record_source
    FROM base b
...
SELECT
    ...
    z.konto_l2_prefix                                                   AS konto_l2_prefix,
    CAST(NULL AS NVARCHAR(10))                                          AS zeilentyp,
    CAST(GETDATE() AS DATETIME2(7))                                     AS dss_load_date,
...
SELECT
    ...
    CAST('=' AS NVARCHAR(10))                     AS zeilentyp,
    CAST(GETDATE() AS DATETIME2(7))               AS dss_load_date,
    CAST('plug' AS NVARCHAR(255))                 AS dss_record_source
