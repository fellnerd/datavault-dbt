{{
  config(
    ...
    post_hook=[
      ...
      "UPDATE sr SET sr.status = 'loaded' FROM mds_stage.staged_record sr INNER JOIN mds_stage.[commit] c ON sr.commit_id = c.id WHERE sr.entity_id = 9 AND sr.status = 'committed' AND c.status = 'approved'",
      "-- Update commit status to 'loaded'",
      "UPDATE mds_stage.[commit] SET status = 'loaded', deployed_at = GETUTCDATE() WHERE status = 'approved' AND entity_id = 9"
    ]
  )
}}
...
{#
  =====================================================
  MDS Load: Jira Projekt
  =====================================================
  
  Entity Code: project
  Entity ID:   9
  Generated:   2026-01-20T19:54:32.907872
  
  Source: mds_stage.staged_record (JSON data)
  Target: mds_load.project (flache Tabelle)
  ...
  Die vollständige Historie wird im Master (mds_master.project)
  via SCD2 geführt.
  =====================================================
#}
...
SELECT
    ...
    sr.business_key,
    JSON_VALUE(sr.data, '$.code') AS code,
