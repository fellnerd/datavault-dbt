{{
  config(
    ...
    post_hook=[
      ...
      "UPDATE mds_stage.[commit] SET status = 'deployed' WHERE status = 'loaded' AND entity_id = 2",
      "-- Remove DELETE records from load (they should not appear in current state)",
      "DELETE FROM mds_load.product WHERE operation = 'DELETE'"
    ]
  )
}}
...
{#
  ...
  Entity Code: product
  Generated:   2026-01-14T18:47:03.160151
    ...
    SELECT 
        ...
        CASE 
            ...
            WHEN s.operation = 'UPDATE' OR (
                COALESCE(CAST(s.product_id AS NVARCHAR(MAX)), '') != COALESCE(CAST(t.product_id AS NVARCHAR(MAX)), '') OR
