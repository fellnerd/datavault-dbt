  ...
  config(
    ...
    pre_hook=[
      ...
      WHERE is_current = 1 
        AND business_key IN (
          SELECT business_key 
          FROM mds_load.project 
          WHERE is_processed = 0 
            AND operation IN ('UPDATE', 'DELETE', 'INSERT')
            AND business_key IN (SELECT business_key FROM mds_master.project WHERE is_current = 1)
        )
      {% endif %}"
    ],
    post_hook=[
      "-- Mark load records as processed",
      "UPDATE mds_load.project SET is_processed = 1, processed_at = GETUTCDATE() WHERE is_processed = 0",
      "-- Update commit status to 'deployed' for all loaded commits",
      "UPDATE mds_stage.[commit] SET status = 'deployed' WHERE status = 'loaded' AND entity_id = 9",
      "-- Remove DELETE records from load (they should not appear in current state)",
      "DELETE FROM mds_load.project WHERE operation = 'DELETE'"
    ]
  )
}}
...
{#
  =====================================================
  MDS Master: Jira Projekt
  =====================================================
  
  Entity Code: project
  Generated:   2026-01-20T19:54:32.908119
  
  Source: mds_load.project
  Target: mds_master.project (SCD2 historisiert)
    ...
    SELECT 
        ...
        CASE 
            ...
            WHEN s.operation = 'UPDATE' OR (
                COALESCE(CAST(s.code AS NVARCHAR(MAX)), '') != COALESCE(CAST(t.code AS NVARCHAR(MAX)), '') OR
