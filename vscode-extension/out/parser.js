...
exports.DbtProjectParser = void 0;
exports.findDbtProjects = findDbtProjects;
...
class DbtProjectParser {
     ...
     */
    inferModelType(relativePath, fileName) {
        ...
        if (pathLower.includes('/business_vault/') || pathLower.includes('\\business_vault\\')) {
            if (nameLower.startsWith('pit_'))
                return 'pit';
            if (nameLower.startsWith('bridge_'))
                return 'bridge';
            return 'table';
        }
        // Check name-based patterns
        if (nameLower.startsWith('hub_'))
            return 'hub';
        if (nameLower.startsWith('eff_sat_'))
            return 'effectivity_satellite';
        if (nameLower.startsWith('sat_'))
            return 'satellite';
        if (nameLower.startsWith('link_'))
            return 'link';
        if (nameLower.startsWith('stg_'))
            return 'staging';
        if (nameLower.startsWith('pit_'))
            return 'pit';
        if (nameLower.startsWith('bridge_'))
            return 'bridge';
        if (nameLower.startsWith('dim_') || nameLower.startsWith('fact_'))
            return 'mart';
        if (nameLower.startsWith('ref_'))
            return 'ref';
        return 'view';
    }
    /**
     ...
     */
    determineMaterialized(content, layer, type) {
     ...
     */
    enrichSatellite(model, allModels) {
        ...
        const attributes = model.columns.filter(col => !metadataColumns.some(prefix => col.toLowerCase().startsWith(prefix)));
        return {
            ...model,
            type: model.type,
            parentHub,
     ...
     */
    enrichPit(model, allModels) {
        const baseHub = model.refs.find(ref => ref.startsWith('hub_')) || null;
        const includedSatellites = model.refs.filter(ref => ref.startsWith('sat_') || ref.startsWith('eff_sat_'));
        return {
            ...model,
            type: 'pit',
            baseHub,
...
exports.DbtProjectParser = DbtProjectParser;
/**
 * Find dbt_project.yml in workspace
 */
async function findDbtProjects(workspaceFolders) {
    const projects = [];
    for (const folder of workspaceFolders) {
        ...
        try {
            const files = await (0, glob_1.glob)('**/dbt_project.yml', {
